        ...
        try {
            if (token.type === "heading_open") {
                ...
                if (nextToken && nextToken.type === "inline") {
                    ...
                    let fontSize;
                    switch (level) {
                        case 1: headingLevel = HeadingLevel.HEADING_1; fontSize = DOCX_STYLES.sizes.h1; break;
                        case 2: headingLevel = HeadingLevel.HEADING_2; fontSize = DOCX_STYLES.sizes.h2; break;
                        case 3: headingLevel = HeadingLevel.HEADING_3; fontSize = DOCX_STYLES.sizes.h3; break;
                        default: headingLevel = HeadingLevel.HEADING_3; fontSize = DOCX_STYLES.sizes.h3;
                    }
                    paragraphs.push(new Paragraph({
                        text: nextToken.content,
                        heading: headingLevel,
                        spacing: { before: DOCX_STYLES.spacing.headingBefore, after: DOCX_STYLES.spacing.headingAfter },
                        style: { font: DOCX_STYLES.fonts.heading, size: fontSize * 2 },
                    }));
                }
                i += 2;
            }
                ...
                if (nextToken && nextToken.type === "paragraph_open") {
                    ...
                    if (inlineToken && inlineToken.type === "inline" && inlineToken.children) {
                        ...
                        else if (listType === "ordered") { bulletText = `${orderedCounter}. `; orderedCounter++; }
                        paragraphs.push(new Paragraph({
                            children: [new TextRun({ text: bulletText, font: DOCX_STYLES.fonts.body }), ...textRuns],
                ...
                if (nextToken && nextToken.type === "paragraph_open") {
                    ...
                    if (inlineToken && inlineToken.type === "inline") {
                        paragraphs.push(new Paragraph({
                            children: [new TextRun({ text: inlineToken.content, italics: true, color: "666666", font: DOCX_STYLES.fonts.body })],
                            spacing: { before: 200, after: 200 },
                            indent: { left: 720 },
                            alignment: AlignmentType.JUSTIFIED,
                            border: { left: { color: "4F46E5", space: 1, style: "single", size: 24 } },
                        }));
                    }
                }
            }
                ...
                paragraphs.push(new Paragraph({
                    children: [new TextRun({ text: token.content, font: "Courier New", size: 20, color: "333333" })],
                    spacing: { before: 200, after: 200 },
                    shading: { fill: "F5F5F5" },
                }));
            }
        ...
        try {
            if (token.type === "heading_open") {
                ...
                switch (level) {
                    ...
                    case 3: fontSize = TYPOGRAPHY.sizes.h3; break;
                ...
                if (i + 1 < tokens.length && tokens[i + 1].type === "heading_close") i++;
            }
            else if (token.type === "paragraph_open") {
                ...
                if (i + 1 < tokens.length && tokens[i + 1].type === "paragraph_close") i++;
            }
            else if (token.type === "bullet_list_open") {
                inList = true;
                listType = "bullet";
                ...
                else if (listType === "ordered") { bullet = `${orderedListCounter}. `; orderedListCounter++; }
                ...
                doc.moveDown(TYPOGRAPHY.spacing.paragraphSpacing / TYPOGRAPHY.sizes.body);
                doc.font("Courier").fontSize(9).fillColor(TYPOGRAPHY.colors.text).text(token.content, { indent: 20, align: "left" });
