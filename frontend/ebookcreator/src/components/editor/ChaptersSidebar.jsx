...
import { ArrowLeft, Sparkles, Trash2, Plus, GripVertical, Grip } from "lucide-react";
...
const SortableItem = ({
  ...
  return (
    <div ref={setNodeRef} style={style} className="group flex items-center bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 relative overflow-hidden">
      <button
        className={`flex-1 flex items-center p-3 text-sm rounded-l-lg text-left transition-colors ${selectedChapterIndex === index
      ...
      <div className="flex items-center ml-2 bg-white opacity-0 group-hover:opacity-100 transition-opacity px-2 py-3 absolute right-0">
        ...
        <Button variant="ghost" size="sm" className="py-2 px-2" onClick={() => onDeleteChapter(index)} title="Delete Chapter">
